...
public class MusicPlayer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (Application.isEditor) absolutePath = "C:/Unity Projects/Kontrol Songs";
        //if (Application.isEditor) absolutePath = "INTERNAL_STORAGE/Music";
    ...
    void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(10,50,100,50), "Previous", myStyle)) {
            Seek(SeekDirection.Backward);
            PlayCurrent();
        }
        if (GUI.Button(new Rect(10,100,100,50), "Play", myStyle))  {
            PlayCurrent();
        }
        if (GUI.Button(new Rect(10,150,100,50), "Next", myStyle))  {
            Seek(SeekDirection.Forward);
            PlayCurrent();
        }
        if (GUI.Button(new Rect(10,200,100,50), "Reload", myStyle))  {
            ReloadSounds();
        }
    }
